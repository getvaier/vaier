...
| **Disk alert threshold** | The percentage of host filesystem capacity in use at or above which Vaier raises **disk pressure** (default 85%). Configurable in Settings. Also governs **remote disk pressure**. |
| **Remote disk usage** | A point-in-time reading of a remote machine's root-filesystem fullness, gathered by running `df` over SSH on a machine behind the VPN rather than reading Vaier's own disk. Carries the machine name and the used percentage. Modelled by `domain.RemoteDiskUsage`. |
| **Remote disk pressure** | The condition where a remote machine's **remote disk usage** is above the **disk alert threshold** — the machine-behind-the-VPN counterpart to **disk pressure**, which is Vaier's own host. |
...
| **Remote disk-pressure alert** | Email sent to every **admin**-role **access entry** when a machine behind the VPN crosses into **remote disk pressure**, with a paired recovery email when it drops back below. The remote-host sibling of the **disk-pressure alert**: it is raised only for SSH-accessible machines Vaier holds a **host credential** for, alerts only on a boundary crossing per machine, and a machine that is unreachable or whose `df` fails is skipped rather than alarmed. |
