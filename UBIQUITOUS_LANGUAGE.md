...
| **Server LAN CIDR** | The IPv4 CIDR of the LAN/VPC subnet the **Vaier server itself** sits on — e.g. an AWS VPC subnet `172.31.0.0/16`. Auto-**discovered** via EC2 IMDSv2 (`network/interfaces/macs/<mac>/subnet-ipv4-cidr-block`); `VAIER_SERVER_LAN_CIDR` overrides for non-EC2 installs; resolved by `ForResolvingServerLanCidr`. A LAN server or LAN service whose address falls inside it is registered, scraped, and published directly from the Vaier-side containers (vaier / traefik → docker bridge → host → the host's LAN/VPC NIC) — no relay peer needed. Empty when undeterminable (the feature is then inert). *(V1 scope: this routes the Vaier-side containers to the subnet; it does not yet add the CIDR to split-tunnel server peers' client-side `AllowedIPs` — tracked as a follow-up.)* |
