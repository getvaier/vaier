...
| **Internet gateway** | A single server peer designated as the central internet egress for full-tunnel VPN clients: the Vaier server forwards all client internet traffic to it instead of out its own connection, so a client appears to be wherever the gateway peer is. At most one peer is the internet gateway at a time. Eligible only for Linux server peers (which run the bash setup script that installs the egress NAT rules). Distinct from a **relay peer** (which carries one LAN, not the internet), and from the presentational device-category `GATEWAY` icon. |
...
| relay peer | bridge peer, gateway (means the internet gateway) |
| internet gateway | gateway peer, exit node, VPN gateway |
