...
| **Backup server** | A machine running a borg server that Vaier can address and that holds **backup repositories**. Its name is an **identifier** (letters, digits, `_` and `-` only); a friendlier label an operator types is slugged to it, so spaces become hyphens. Vaier can provision one from nothing on a chosen machine, or register an existing one, and can trust each client machine's SSH key on it so backup jobs authenticate. |
...
| **Backup job** | A per-machine **fleet backup** specification: which machine's data to back up, into which **backup repository**, from which source paths, with which excludes, retention, compression, whether it is enabled, and whether it backs up **as root**. |
...
| **Entry** | One addressable thing in the **Explorer** — a machine, a directory, a file, a shell. Every entry has a **path**, and the **Inspector** renders it by what it is. A **file entry** is one kind of entry. |
...
| **Rail** | The tree column of the **Explorer**, down the left. It carries **structure**: the fleet, its machines, and the directories inside them — directories become entries in the rail, files do not. What a directory *contains* is the **Inspector**'s job, not the rail's. |
| **Inspector** | The pane of the **Explorer** that renders the selected **entry**, chosen by what that entry is: a machine shows its details, a directory shows its listing (files and directories alike), a shell opens a terminal. |
| **Path** | Where an **entry** lives. On a machine's filesystem it is always absolute, and always the real location: redundant separators and `.`/`..` steps are resolved away, and a path that would climb above the root is not a path at all. |
