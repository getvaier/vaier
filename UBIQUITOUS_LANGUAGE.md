...
| **Description** | Optional free-text label an operator attaches to a machine — a VPN peer or a LAN server — to record what it is, e.g. "Home media server (NUC, Ubuntu 22.04)". Purely informational, and distinct from the machine **name** (its identity). |
| **Rename** | Changing the **name** of a peer or LAN server in place. The machine keeps its identity, configuration, and published services — only the label changes. Distinct from deleting and recreating. |
...
| **VAIER metadata** | The `# VAIER: {...}` JSON comment at the top of every client WireGuard config. Carries `peerType`, `lanCidr`, `lanAddress`, `description`. Legacy peers without it default to `UBUNTU_SERVER`. |
...
| **Path prefix** | Optional `pathPrefix` on a published service (e.g. `/auth`). When set, the Traefik rule becomes `Host(...) && PathPrefix(...)` so multiple services can share one subdomain, each routed by path. Normalised by `ReverseProxyRoute.normalisePathPrefix` (trailing slash stripped, blank/"`/`" → null). Route uniqueness is the `(fqdn, pathPrefix)` pair. |
...
| **Launchpad alias** | Optional operator-supplied tile label (`launchpadAlias`, persisted as `x-vaier-launchpad-alias` map). Overrides the default label when non-blank. Settable via `PATCH /published-services/{dnsName}/launchpad-alias`. |
...
| **Favicon lookup identity** | What the launchpad uses to ask `/favicon` for an icon. Computed by `ReverseProxyRoute.launchpadFaviconQuery()`: host-only routes use just the FQDN; path-based routes carry `pathPrefix` too, so siblings under one host don't collide on the favicon cache and the CDN-by-name fallback uses the path segment instead of the shared subdomain. The launchpad client renders `service.faviconQuery` opaquely — it never decides what's in it. |
...
| **Tile** | A single service card on the launchpad — service name, peer name, favicon, link, and, on hover, the Docker image and running version of the service behind it. |
| **Backing container** | The Docker container that serves a published service. Its image and version are surfaced on the launchpad tile, resolved by `ReverseProxyRoute.backingContainer(...)` against discovered peer / Vaier-server / LAN-server containers. A service published as a bare LAN host:port has no backing container. |
| **Version endpoint** | An operator-configured URL and property name on a published service. Vaier reads the service's running version from that URL and shows it on the tile — used for a service that has no backing container, such as one running natively on a LAN machine. |
...
| **Caller authenticated** | Whether the launchpad viewer holds a valid Authelia session. Determined by which endpoint served the request, not by an inspected header: `/launchpad/services` is in Authelia's bypass set and always passes `callerAuthenticated=false`, while `/launchpad/services-authenticated` falls through to Authelia's `one_factor` policy — reaching it is itself proof of a valid session, so the controller hardcodes `true`. The launchpad page tries the authenticated endpoint first and falls back to the public one on a redirect to login. Auth-protected routes are hidden from anonymous viewers (issue #207). |
