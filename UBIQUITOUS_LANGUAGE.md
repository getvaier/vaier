...
| **Command result** | The outcome of a **remote command**: the process exit code (or unknown when it timed out or the server sent none), the captured stdout and stderr, and whether it timed out. A non-zero exit code is a normal result the caller inspects, not an error. |
