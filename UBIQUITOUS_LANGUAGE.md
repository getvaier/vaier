...
| **Relay peer** | A server peer with a `lanCidr` set. Vaier routes traffic for that CIDR into the relay's tunnel, and the relay's install script enables `ip_forward` and idempotent iptables NAT/FORWARD rules so packets reach LAN hosts behind it. The rules are re-applied on every boot by a generated `vaier-wg-relay-iptables.service` systemd oneshot. |
