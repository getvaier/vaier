...
| **Send password** | Sending a machine's stored password from Vaier straight into an open **web terminal**, so it reaches the SSH server without the browser ever holding it. Only done when the remote is at a **password prompt**, so it cannot echo into the screen or shell history. |
| **Password prompt** | The remote shell asking for a password (a login, a `sudo`, a further `ssh` hop), recognised from the tail of the terminal's recent output. The one moment at which it is safe to **send password**. |
...
| **Prepare client** | Installing the borg client on a **backup client** that lacks it, so its **backup runs** don't fail with "borg not found". Vaier installs it itself where it can gain root over SSH, or hands the operator one command to run where it can't. A run whose machine has no borg is refused up front with a message pointing here. |
