...
| **Name** | A machine's human-readable display label — what the UI shows. For a peer it is free-form text the operator edits at will; editing it changes only this label, never the **peer id**. A peer with no stored name falls back to its **peer id** rendered with dashes as spaces. Distinct from the **peer id** (a peer's identity) and the **description** (informational). |
