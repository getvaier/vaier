...
| **Map tab** / **List tab** / **Topology tab** | The three views on the Machines page. The List tab shows machine cards; the Map tab renders a self-hosted Leaflet/OpenStreetMap world map with markers and clustering; the Topology tab renders the **topology diagram**. |
| **Topology diagram** | The interactive, force-directed bird's-eye view of the whole Vaier network shown on the Machines page's **Topology tab**: the Vaier server, every VPN peer, each LAN server, and every published service are nodes, with each published service attached to the machine that hosts it. A physics simulation lays the nodes out, and they can be dragged, zoomed, and panned. Each node is coloured by its own health and edges are coloured and dashed by connectivity status, reusing the same machine icons and status colours as the List and Map tabs. |
