...
| **Avatar** | The image beside each **access entry** in the access overview: the person's provider photo when available — the GitHub account picture (via the **provider user id**), otherwise a Gravatar keyed on the email — falling back to a **monogram** (initials on an email-derived colour) when no photo loads. |
