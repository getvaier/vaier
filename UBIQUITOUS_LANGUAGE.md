...
| **Infrastructure** | The operator's machine-and-service management surface. No longer a distinct page or admin-shell section: adding, monitoring and editing machines, the world **Map**, publishing and managing published services, the **LAN scanner**, **host credentials** and setup scripts all live in the **Explorer** now, and the former Infrastructure page is gone. Retained only as the label the launchpad and old links use for the Explorer entry point. Earlier still it replaced the separate **Machines** and **Services** pages, which also no longer exist. |
...
| **Capability strip** | Retired with the Infrastructure page's machine cards. Was the card-header row of capability icons (relay, docker); the **Explorer** conveys a machine's capabilities through which entries it grows instead. |
...
| **Web terminal** | A live, in-browser SSH shell to a machine, opened from its **shell** entry in the **Explorer**. Vaier authenticates server-side from the credential vault; the browser only exchanges keystrokes and screen output, never the secret. |
