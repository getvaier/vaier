...
| **Entry** | One addressable thing in the **Explorer** — a machine, a directory, a file, a shell, a container, a **published service**, a machine's disk. Every entry has a **path**, and the **Inspector** renders it by what it is. A **file entry** is one kind of entry. A machine holds only the entries Vaier can actually reach on it, so two machines need not hold the same ones. |
