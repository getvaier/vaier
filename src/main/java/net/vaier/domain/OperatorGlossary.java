    ...
    public static List<ConceptGroup> groups() {
        return List.of(
            new ConceptGroup("Machines", List.of(
                ...
                Concept.of("Internet gateway",
                    "A single server peer you designate as the central internet exit for full-tunnel VPN clients, so their internet traffic leaves through that peer instead of the Vaier server.",
                    "Pick one to make clients appear to be at the gateway's location, e.g. a box in another country. Only one at a time, and the peer must have run its setup script."),
