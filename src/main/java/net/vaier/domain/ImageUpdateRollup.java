 ...
 *
 * @param images the images newly found to have an update available, in the order they should be listed
 */
public record ImageUpdateRollup(List<String> images) {
    ...
    /**
     * Subject line. It names the image when there is one — the operator can act straight from the subject —
     * and counts them when there are several, since a subject listing eight image tags is unreadable.
     */
    public String subject() {
        if (images.size() == 1) {
            return "[Vaier] Update available: " + images.get(0);
        }
        return "[Vaier] Update available: " + images.size() + " images";
    }

    /**
     * Body: every image that went stale, then the one thing the operator needs to know — that nothing will
     * happen until they act. {@code baseDomain} builds the Vaier UI link, omitted when null or blank.
     */
    public String body(String baseDomain) {
        ...
        for (String image : images) {
            body.append("  • ").append(image).append("\n");
        }
