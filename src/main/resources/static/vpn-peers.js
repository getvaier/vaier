...
// inline on* handlers keep resolving. Modularisation tracked as follow-up slices of #273.
        ...
        // --- Web terminal (#306/#308) ---
        // The terminal itself now lives in the admin shell (admin.html + terminal-dock.js) as its own
        // top-nav tab, so its live SSH sessions survive navigating away from Infrastructure. A machine's
        // Terminal button just asks the shell to open a shell for that machine; the dock does the rest.
        function openTerminalForMachine(machineName) {
            const shell = window.parent;
            if (shell && shell !== window && typeof shell.vaierOpenTerminal === 'function') {
                shell.vaierOpenTerminal(machineName);
            } else {
                // vpn-peers.html is meant to run inside the admin shell; opened on its own there is no dock.
                console.warn('The web terminal is only available inside the Vaier admin shell.');
            }
        }
