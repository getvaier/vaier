...
class NotificationServiceTest {
    ...
    private net.vaier.domain.DiskUsage diskUsage(long totalBytes, long usableBytes) {
        return new net.vaier.domain.DiskUsage("/host", totalBytes, usableBytes);
    }
    ...
    void notifyAdminsOfDiskPressure_sendsAlertToEveryAdminWithSubjectAndBody() {
        ...
        assertThat(recipients.getValue()).containsExactly("alice@example.com");
        assertThat(subject.getValue()).isEqualTo("[Vaier] Host disk is 90% full");
        assertThat(body.getValue()).contains("90%");
        assertThat(body.getValue()).contains("/host");
        assertThat(body.getValue()).contains("85%");
    }
    ...
    void notifyAdminsOfDiskRecovery_sendsRecoverySubject() {
        ...
        verify(emailSender).sendEmail(any(), anyInt(), any(), any(), any(),
                anyList(), subject.capture(), any());
        assertThat(subject.getValue()).isEqualTo("[Vaier] Host disk is back to 50% full");
    }

    @Test
    void notifyAdminsOfDiskPressure_skipsWhenSmtpNotConfigured() {
        when(configPersistence.load()).thenReturn(Optional.of(VaierConfig.builder().build()));

        service.notifyAdminsOfDiskPressure(diskUsage(100L, 10L), 85);
