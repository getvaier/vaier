...
| **Internet gateway** | Designate one Linux server peer as the central internet egress for your full-tunnel clients: their internet traffic then leaves through that peer instead of the Vaier server, so a phone on the VPN appears to be wherever the gateway peer is. One gateway at a time — picking a new one clears the old. The peer must have run its setup script first (it installs the egress NAT rules). |
...
A Linux server peer's card also offers an **Internet gateway** toggle. Designate one such peer as the internet gateway and the Vaier server stops NATing full-tunnel client internet traffic out its own connection — it forwards that traffic to the designated peer instead, which masquerades it out to the internet. A client on the VPN then appears to be wherever the gateway peer is (handy for a box in another country). Only one peer can be the internet gateway at a time: picking a new one automatically clears the previous. The change is server-side only — no client reconfiguration — but the chosen peer must have already run its **setup script**, which installs the persistent egress NAT rules every Linux server peer carries. _(V1 limitation: a single, all-clients gateway; per-client selection and multiple simultaneous gateways are planned.)_
