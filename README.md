...
| **Email notifications** | SMTP settings in *Settings* power Authelia password-reset emails and Vaier admin alerts when a server peer connects or disconnects (sent to every user in the `admins` group; the first observation after Vaier startup is treated as a baseline so restarts don't generate noise). Credentials are verified against the server before saving; a dedicated "Send test email" button delivers a real roundtrip message to any recipient. |
