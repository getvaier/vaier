...
| **LAN scanner** _(Enterprise)_ | When adding a **LAN server**, scan its relay's LAN right from the Add Machine dialog to discover hosts and **adopt** one into a registered LAN server in a single call — Vaier derives the name, address, Docker settings, and device icon from the scan, so you only confirm (or rename). You can also attach an SSH credential as you adopt, and **test** it first — Vaier opens a throwaway connection to prove the login works (and never stores a credential it couldn't verify), all without echoing the secret. Already-registered machines are filtered out, so only new hosts appear. Requires an Enterprise licence. |
