...
| **Explorer** | Browse the files on any SSH-capable machine in the fleet — one file tree spanning every peer, LAN server, and the **Vaier server** host itself. Vaier reads the machine's filesystem over SFTP, authenticating server-side from the **credential vault** and trusting the host key by the same trust-on-first-use pin as the web terminal, so browsing a machine needs no new credentials and no new trust. Directories list before files, and a path that isn't absolute — or that tries to climb above the root — is refused before Vaier opens any connection. Read-only for now: moving files between machines, downloading them to your browser, and browsing a machine's backup **archives** as of a point in time all land in later slices. |
