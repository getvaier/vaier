...
| **Update available** | Vaier tells you when a container is running an image its registry has since moved on from. Once a day it compares the digest the running container's image actually has against the digest that registry serves for the **very same tag** today — a difference means an **update available**. Any Registry v2 registry works, not just Docker Hub: `ghcr.io` and `lscr.io` are read the same way, with no registry account, token or config to supply. **Vaier never pulls and never restarts anything** — detection is read-only, and updating stays your call. When an image *newly* goes out of date, every admin gets **one rollup email** listing what changed (three images going stale in the same sweep is one mail, not three; nothing changed is no mail at all) — and unlike the disk and machine alerts, an image that is already out of date the first time Vaier looks *is* reported, because a quietly-stale image is exactly the incident this exists for. What Vaier can't tell, it says: an unreachable or rate-limited registry, a locally-built image with no registry digest, or an image pinned to an exact digest all read as **unknown** — never as up to date, and never as out of date. Checks run daily rather than continuously because registry manifest requests are rate-limited, and the 30-second container scrape never touches a registry. This first slice covers the Vaier server's own containers and those on your VPN **server peers**; LAN-server containers read as unknown for now. The verdict rides on the existing container payloads as `updateAvailable`. In the **Explorer**, a container with an **update available** wears a small yellow mark — in the tree beside its name, and in its machine's container list — so you can spot it while scanning, without opening anything. The mark is advisory, not an alarm: nothing is broken, and red stays reserved for down. **Unknown draws no mark at all** (it is the ordinary resting state, and a permanent grey smudge on every row would just teach you to ignore it) — so no mark is *not* a promise that an image is current. Where that difference matters, a container's own Inspector says which of the three verdicts it is, in words, including "Vaier cannot tell". |
