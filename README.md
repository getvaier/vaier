...
| **Reverse proxy & edge hardening** | Traefik terminates HTTPS with Let's Encrypt, enforces a security-header and TLS floor on every route, and shows a branded offline page when a backend is down. CrowdSec blocks malicious traffic at the edge; every block is listed in the Explorer, one click to lift it, even your own, or trust the address. → [Networking](docs/NETWORKING.md#edge-hardening) |
