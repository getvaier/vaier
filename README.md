...
| **Monitoring & alerts** | Disk watching with a fill-rate forecast that mails you about a week before a disk fills, image-update detection with a one-click **Update**, a self-check that names anything in Vaier's own reverse proxy config no route can reach, and an inbox that stays quiet unless something is wrong. → [Monitoring](docs/MONITORING.md) |
