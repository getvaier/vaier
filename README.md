...
| **Fleet backup** _(Enterprise)_ | Back up every machine in your fleet to a [borg](https://www.borgbackup.org/) repository on your NAS. Configure the repository once (NAS host, borg SSH port, user, path, an encrypting passphrase, append-only flag), give each machine a **backup job** (source paths, excludes, retention, compression), and run it on demand with **Run now** or let Vaier's nightly schedule do it. Guided provisioning checks host readiness (is borg installed and a supported version, can the machine reach the NAS) and can initialise the repository; browse the archives it writes. A failed run emails all admins. Restore from the UI isn't available yet. Requires an Enterprise licence. |
...
_(Enterprise)_ Vaier can back up the machines in your fleet to a [borg](https://www.borgbackup.org/) repository on your NAS. Open the **Backups** tab in the console (Community instances see the tab in a locked state, since it's an Enterprise feature).

**Backup repository** — Configure the store once: the NAS host, the borg SSH port (default `8022`), the borg user (default `borg`), the repository path, an encrypting **passphrase** (stored encrypted at rest and never shown back), and an append-only flag. Guided provisioning does the setup for you — **Check host readiness** reports whether borg is installed on a machine (and which version, and whether it's supported) and whether that machine can reach the NAS, and **Initialise repository** runs `borg init` from a machine that references it.

**Backup jobs** — Give each machine a job: which machine (a VPN peer, by name), which backup repository, the source paths to back up, exclude patterns, retention (`keepDaily` / `keepWeekly` / `keepMonthly`), compression (default `zstd,6`), and whether the job is enabled.

**Running** — Run any job on demand with **Run now**, or let the **nightly schedule** run every enabled job once a day. The schedule hour (0–23, default 2) is set on the ungated **Settings** surface, alongside the disk-pressure threshold. Each execution is a **backup run** with a status (running, success, failed, or unknown); a failed run emails every admin, reusing the same SMTP configuration as the other alerts. Vaier keeps its small per-host working state — the borg passphrase file and each run's result/log — in `~/.vaier-backup` on the target machine (the SSH user's own home, so it works whether or not that user is root), falling back to `/tmp/vaier-backup` if the home directory can't be determined.
