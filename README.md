...
| **Web terminal** | Open a real SSH shell to any SSH-capable machine — VPN peers, LAN servers, and the **Vaier server** host itself — right from its Infrastructure card, with a **Terminal** button. Each terminal is its own draggable, resizable floating window, so you can keep several shells live at once (even two to the same host) and arrange them side by side; resizing reflows the remote PTY to match. Vaier authenticates server-side from the **credential vault** (the browser never sees the secret) and pins each host's key on first use, refusing a later mismatch. Distinct, legible failures when there's no credential, the host is unreachable, auth fails, or the host key changed. Powered by a local xterm.js over a WebSocket and Apache MINA sshd. |
