...
| **Host credentials** | Store the one SSH login Vaier holds for each machine — a username plus a password or private key (with optional passphrase) — from the machine's card on the Infrastructure page. Each card has an **SSH access** toggle that decides whether Vaier offers SSH for that machine; it defaults sensibly from the device type (servers and NAS on, printers/phones/appliances off) and the credential control only appears when SSH access is on. Secrets are encrypted at rest in a **credential vault**; the UI only ever reports whether a credential exists, never the secret itself. This is the foundation for the upcoming per-host **web terminal** (interactive SSH from the browser). |
