...
| **Disk monitoring** | Vaier watches free space on every SSH-capable machine it holds a credential for, including the Vaier host itself via SSH-to-self (via `df` over SSH), emailing all admins when usage crosses a configurable threshold (default 85%), with a recovery email once it drops back below. You don't have to wait for the email to see the number: a machine's **disk** entry in the **Explorer** takes the same reading on demand and shows it against the same threshold. |
