...
| **Is it working?** | Settings runs a pre-flight over Vaier's own basics — the wildcard record, the certificate on its front door, the tunnel, the disk under it — and shows only what is wrong, with what to do. A healthy server shows nothing. → [Monitoring](docs/MONITORING.md#pre-flight) |
| **Chat** | Ask about your fleet in plain sentences and Marvin answers — from Vaier's own facts, a read-only command, or a public web page when neither covers it. He acts only on your yes, hands you files, and runs errands: send him off to check something later and he mails the answer, or a link to approve what he found. → [Chat](docs/CHAT.md) |
