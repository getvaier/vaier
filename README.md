...
| **Access management** | Day one needs no OAuth app: the first-run password in Vaier's log makes the first sign-in the admin. Add Google or GitHub from Settings to invite others, with roles and per-service access groups — and a gated service can be handed its own login, so nobody needs its password. → [Auth](docs/AUTH.md) |
