...
- **Server LAN CIDR — Vaier server as its own LAN router** ✅ — the Vaier server declares the CIDR of the subnet it sits on (typically an AWS VPC subnet), so machines in that subnet can be registered as LAN servers and have their services published *without a relay peer*. The CIDR is **discovered**, not configured: `ForResolvingServerLanCidr` reads EC2 IMDSv2 (`network/interfaces/macs/<mac>/subnet-ipv4-cidr-block`), with `VAIER_SERVER_LAN_CIDR` as a non-EC2 override; anything that doesn't parse as a strict IPv4 CIDR (`Cidr.validateLanCidr`) is ignored. Such a machine is anchored at `"Vaier server"` (`LanAnchor.VAIER_SERVER_NAME`) — it surfaces on the Machines page (`Machine.lanCidr` = the server CIDR), shows "via Vaier server", is reachability-probed and Docker-scraped straight from the Vaier-side containers (vaier / traefik → docker bridge → host → the host's LAN/VPC NIC, which already works because Docker enables `ip_forward` on the host and masquerades the bridge network out the host NIC), and publishes a normal `isLanService` Traefik route. When an address is covered by both a relay peer's `lanCidr` and the server LAN CIDR, the relay peer wins. **V1 scope:** this routes the Vaier-side containers to the subnet; it does *not* yet add the server CIDR to split-tunnel `UBUNTU_SERVER`/`WINDOWS_SERVER` peers' client-side `AllowedIPs` (full-tunnel client peers already reach it), nor install a VPN→VPC `iptables` masquerade on the Vaier host — tracked as a follow-up issue. No Settings-UI field and no `vaier-config.yml` entry: env + IMDS cover the supported cases.
