...
- **LAN Docker host discovery** ✅ (closes [#177](https://github.com/getvaier/vaier/issues/177)) — register a Docker host that lives on a relay peer's LAN (a NAS that runs Docker, a Mac mini, an extra Docker server) without making it a VPN peer. Vaier scrapes its remote Docker socket through the relay (same `tcp://<host>:2375` pattern as VPN peers), surfaces the discovered containers in the publishable list with a `LAN_DOCKER_HOST` source, and renders the host on the Machines page with a "LAN docker" badge and resolved relay attribution. Registration validates that the host IP falls inside some relay peer's `lanCidr`. Persisted as YAML at `${VAIER_CONFIG_PATH}/lan-docker-hosts.yml`. V1 scope: insecure tcp 2375 only; no TLS/SSH yet.
