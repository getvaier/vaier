...
**Add a sign-in provider from Settings (slice 2 of [#264](https://github.com/getvaier/vaier/issues/264), open).** Slice 1 got a newcomer into the console with no OAuth app at all (shipped 2026-09-22, above), and it ends where the **first-run password** does: to invite a second person the operator must open `.env` over SSH, paste a client id and secret, and run `docker compose up -d` so `dex-init` and Dex pick them up. Settings should take those two fields instead and make them live — which means Vaier writing a provider's credentials somewhere `dex-init` reads, and restarting Dex (and oauth2-proxy's sign-in template) from inside the stack it is itself gated by. The hard part is that last clause: the console is behind the very chain being reconfigured, so a bad value must not be able to lock the operator out, and the first-run door has already closed by the time this flow is used. Until it is built, the docs teach the `.env` path and say so plainly.
