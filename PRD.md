...
- **Per-host LAN setup script** ✅ (closes [#249](https://github.com/getvaier/vaier/issues/249)) — one script the operator runs on a registered LAN server, `GET /lan-servers/{name}/setup.sh`, that **adapts to that host**: it opens the Docker engine API (native + snap, the same daemon.json/systemd-drop-in logic as before) when the host runs Docker, **and** installs static routes via its relay peer when it is relay-anchored. Routes cover the server LAN CIDR, the VPN subnet, and every *other* relay peer's `lanCidr` (so a host behind one relay can reach the Vaier VPC and other sites' LANs), persisted across reboots by a `vaier-lan-routes.service` systemd oneshot (`ip route replace … via <relay lanAddress>`, distro-agnostic, idempotent — mirrors the relay's `vaier-wg-relay-iptables.service`). Strict hex: the decision of what the script must do (effective Docker port, relay-anchor → gateway, routed-CIDR set, "relay has no LAN address" → 409, "nothing to do" → 404) lives in the pure domain `LanServerSetupScript.forHost(...)` / `routedDestinations(...)`; `LanServerService` only reads the `LanServer`, peer configs, server LAN CIDR and VPN subnet from driven ports and hands them over. This **replaces** the old generic `docker-setup.sh` (static classpath script + `--port`) — register-first, since registration never needed Docker reachable; the old endpoint and `scripts/lan-docker-setup.sh` are retired. Sibling-site LAN routes carry traffic since site-to-site routing ([#250](https://github.com/getvaier/vaier/issues/250)) shipped. The endpoint is on oauth2-proxy's **public** path (skip-auth) so `curl … | sudo bash` works on a fresh LAN host with no Vaier session — the script carries no secrets.
- **Site-to-site routing** ✅ (closes [#250](https://github.com/getvaier/vaier/issues/250)) — a host on one relay's LAN reaches a host on another's. **B:** every server peer's client `AllowedIPs` now carries each **sibling relay LAN** after the VPN subnet and server LAN CIDR — decided in the domain by `WireGuardPeerConfig.siblingRelayLans(ownLanCidr, ownLanAddress, fleet)`: normalised, de-duplicated, sorted (so a render never depends on read order), and never a LAN that overlaps the peer's own `lanCidr` or holds its `lanAddress` (unknown address → listed; the setup-script guard is the backstop). `VpnService` hands the peer list to create, Reissue and the out-of-date check, so a relay joining marks every other server peer's config out of date. **C:** a relay's `PeerSetupScript` forwarding is derived from its own config's `AllowedIPs` (the networks its tunnel accepts): per network, MASQUERADE + FORWARD accept into the LAN, and FORWARD accept from the LAN out (replacing the old RELATED,ESTABLISHED-only return rule, so LAN hosts can start connections under Docker's `FORWARD DROP` policy); the `vaier-wg-relay-iptables` oneshot re-applies the same rules and the `vaier-relay` DF-clearing table stays. The far relay masquerades, so far-LAN hosts need no configuration and do not see real client addresses. The hub is unchanged: its `FORWARD -i wg0 ACCEPT` passes relay-to-relay traffic and neither masquerade rule matches wg0 egress. Rollout per relay: Reissue + run the setup link.
  - *Backlog:* surface **out-of-date config** in the Explorer (computed and served, read by no screen), so a stale relay after a fleet change is visible without asking the API.
  - **A Reissue keeps every metadata field it does not render** ✅ — it used to rewrite the whole `# VAIER:` line from the fields `vaierJson` knows and drop `sshAccess`, so a relay whose device category is an appliance (e.g. `GATEWAY`) fell back to SSH access *off* — exactly the relays this rollout reissues. `WireGuardPeerConfig.reissue` now copies every field outside the rendered set onto the new line unchanged.
...
- **Relay peer survives a Docker daemon restart** ✅ 2026-09-21 — the peer setup script installs a root-owned `custom-cont-init.d/10-clear-stale-wg0` script and mounts it into `wireguard-client`, so a stale host-netns `wg0` is deleted before every tunnel start. Written after Colina 27 went dark for 54 minutes: snapd refreshed the `docker` snap, the daemon restart killed the container without `wg-quick down`, `wg0` outlived it, and the restarted container failed with `wg0 already exists`, tore the surviving tunnel down and sat idle with exit code 0 — nothing to restart, dead until a reboot. Vaier's own alerting was fine (disconnected mail two minutes in); the generated stack was the bug, and the script even printed "survives Docker restart" at the end. Peers set up earlier need the file and the mount added by hand or a re-run of the setup link.
...
- ✅ **Vaier's own update carries the stack, not just the image** ([#343](https://github.com/getvaier/vaier/issues/343), §6.61). Distinct from the detection above: this is the **self-update**. It runs `install.sh` at the commit the new image was built from, runs `docker compose up -d` for the whole project, and on failure puts the previous image and runtime files back together. Operator state (`.env` values, every runtime dir) is out of scope, the same as in `install.sh`.
...
- **Driving** — `JudgeContainerStandingsUseCase` on `ContainerService` turns the three caches the scheduler has just refreshed (Vaier's own stack, every server peer, every LAN server) into observations and hands them to the tracker; `StateRefreshScheduler` switches on the verdicts and calls `NotifyAdminsOfContainerTroubleUseCase` on the existing `NotificationService`, each mail in its own try/catch. Vaier's own host is covered too — the masquerade sidecar dying after a deploy is this exact bug on the Vaier server — and an *empty* local reading counts as no answer, since Vaier itself runs in a container there.
...
- **It delivers the alert half of [#317](https://github.com/getvaier/vaier/issues/317)** ("Docker health over SSH") without an SSH channel or a new port on any machine: the Docker-API scrape Vaier already makes over the tunnel is enough to say that a container which was running has stopped. See the addendum below for the rest of #317.

**Addendum 2026-09-11 — unhealthy and restart-looping, off the same reading** ✅ (closes [#317](https://github.com/getvaier/vaier/issues/317)). #356 gave Vaier one word about a container: **gone**. A container can be in trouble long before that, and the listing Vaier already reads says so — its own health check is failing, or Docker is restarting it in a loop. The loop is the predictive case the issue asked for, and the one that may never become "gone" at all: `restart: always` keeps bringing a container back, so nothing would ever have been said about it.

- **The SSH channel #317 proposed was deliberately not built.** Every Docker machine in this fleet already exposes its engine to Vaier, and the 30-second scrape already lists every container — so a second telemetry channel over SSH would have been a new port, a new adapter and a per-machine bounded command for facts already on the wire. The rule this follows is the one in `CLAUDE.md`: do not reach for machinery a feature has not earned. `2375` also stays open, as [#317](https://github.com/getvaier/vaier/issues/317) itself already settled — SSH-gated Docker needs docker-group access on every host, which is a worse onboarding cost than the port it removes.
- **Health comes off the listing, never an inspect.** Docker writes the health verdict into the status line `docker ps` already returns — `Up 2 weeks (healthy)`, `Up 3 minutes (unhealthy)`, `Up 10 seconds (health: starting)`. `domain.ContainerHealth.fromStatus` parses it (unhealthy matched *before* healthy, since one contains the other) and `DockerServerAdapter` fills it on the single builder all three discovery paths share. An inspect per running container would have been one round-trip per container per machine every 30 seconds for a fact already in hand.
- **Four standings, one order of badness.** `ContainerStanding` becomes `RUNNING < UNHEALTHY < RESTARTING < GONE`, and `ContainerStanding.read(DockerService)` owns the precedence — restarting before not-running (a restart loop is not running either, and "gone" is the least useful true thing to say about it), not-running before unhealthy. `health: starting` returns **empty**: the verdict has not been taken, so the standing is held exactly where it was rather than counted either way.
- **Each trouble counts on its own.** `MachineContainerStanding` gains `reading` (what the scrapes are seeing) beside `standing` (what Vaier has said), so one unhealthy scrape followed by one restarting scrape is not two misses of anything, and `troubledSince` dates the trouble that is happening now. Two consecutive answered scrapes of the *same* reading still gate everything, and a container never seen running **well** raises nothing ever.
- **Worse news is news; better news waits for the all-clear.** `isEscalation()` mails only when the new standing is worse than the one already said: unhealthy→gone earns its own mail, gone→unhealthy earns none (the card and badge move anyway, so the surface stays honest), and the single recovery mail lands when it is well again. Without that rule the two would trade mails back and forth across one bad afternoon.
- **Words per trouble.** `MachineContainerStanding` writes *"webtrees is unhealthy on Apalveien 5"* / *"…is restart-looping…"* / *"…is not running…"*, with last-seen-well, trouble-since, and the plain sentence that Vaier changed nothing and cannot restart it. The reboot sentence appears on the **gone** mail alone — a reboot explains a container that did not come back and explains nothing about a failing health check.
- **Surface.** `MachineNudge.Kind.CONTAINER_GONE` becomes `CONTAINER_TROUBLE` and carries the standing's own title, evidence and action sentence. In the Explorer a troubled container reads `gone` / `unhealthy` / `restarting` in its state column, off the same `GET /machines/container-standings` and the same `container-standing-changed` push; a container that is up and unwell wears the shell's existing **amber** (`is-degraded`) rather than a third colour, red staying for one that is not running. Its Inspector says `running (unhealthy)` on one line, so the list and the pane can never disagree.
- **The file tolerates its own past.** `container-standings.yml` gains `reading` and renames `notRunningSince` to `troubledSince`; the old key is still read, and an unknown standing name reads as `RUNNING` — a memory Vaier cannot read must cost a re-learn, never an inbox full of invented alerts.
...
- **#313 — Remote SSH exec → host telemetry & alerts** (umbrella) ✅ closed 2026-09-11 — every slice done (#314–#317; #318 stands alone as backlog). Turn the SSH capability into a
  ...
  - **#317 — Docker health over SSH + close the `apalveien5` 2375 hole.** ✅ Delivered in full over the existing Docker-API scrape, with no SSH channel and no new port: *exited* by the container-standing work of [#356](https://github.com/getvaier/vaier/issues/356), *unhealthy* and *restart-looping* by the 2026-09-11 addendum beside it (both in §6.9). The SSH channel the issue proposed was deliberately not built — the facts are already on the wire — and closing `2375` stays out, as the issue itself settled: SSH-gated Docker needs docker-group access on every host.
...
- **Shells already running on a machine — list, reattach, end ✅ 2026-09-22 (closes [#322](https://github.com/getvaier/vaier/issues/322)).** Vaier created persistent shells but could never enumerate them, so a pane id lost anywhere was a shell lost forever. `PersistentShell.listCommand()` asks tmux for every session led by the machine's own clock, and `readShells` keeps only the `vaier-` ones (the operator's sessions are never listed or touched), with age, time since a window last held it, and whether one holds it now; a narrow `ListPersistentShellsUseCase` on the existing `TerminalService`, never throwing on the open path; `GET/DELETE /machines/{id}/shells[/{paneId}]`. The pop-out window asks once it has an identity and shows a **Shells** bar button only when the machine runs shells besides its own; the sheet says what each runs and whether it is held or orphaned, **Reattach** opens it through the Duplicate door on its own pane id (adopted into the pane store, so it is owned again), **End** is two-tap. The dock the issue sketched is gone, so the list lives in the one shell UI that remains.
- **LAN address is a dotted quad or nothing ✅ 2026-09-22 (#342's precondition).** `PATCH /vpn/peers/{id}/lan-address` accepted any string; the launchpad's local path is `http://<lanAddress>:<port>`, which survives HSTS only while the host is an IP literal (RFC 6797 §8.1.1). A shared domain rule `LanAddress.validate` now refuses hostnames and malformed quads at both the peer and LAN-server boundaries, without resolving anything — the old `InetAddress` check did a DNS lookup on a request path and let hostnames through. HSTS itself stays parked at the operator's request.
...
**Backlog (deferred):**
- **The Vaier server as a backup client ✅** — the Vaier server is the one machine in the fleet that *is* the Docker host rather than something behind the tunnel, so it inherited no relay LAN routes: Traefik's route loop and `LanRouteAdapter` write into container network namespaces, never the host's. A backup job for the Vaier server therefore failed every readiness check — `borg info` could not reach the backup server because the *host* had no route to the relay's LAN (`ip route get 192.168.3.3` left via the default gateway). The `host-lan-routes` service now mirrors Traefik's route loop into the host's own netns, installing each relay peer's `lanCidr` via the wireguard container. Because the host cannot use Docker DNS, wireguard's bridge address is **pinned** (`x-wireguard-bridge-ip`, high in the subnet so Docker's sequential allocator can never collide with it) rather than mounting the Docker socket into a `NET_ADMIN` host-netns container. `wireguard-masquerade` already SNATs `172.20.0.0/16` out of `wg0`, and a host packet routed at the bridge is sourced from `172.20.0.1`, so return traffic needs no extra rule. borg itself still has to be installed on the host via **Prepare client**, and its key trusted via **Authorize host**, like any other client.
...
**One authority for where a blocked address sits ✅ 2026-09-22 (closes [#347](https://github.com/getvaier/vaier/issues/347)).** Blocked addresses were placed by CrowdSec's bundled GeoLite2 while machines were placed by Vaier's DB-IP — two databases on one map, and CrowdSec's `0`/`0` sentinel forced the null-island carve-out above. `BlockDecision.placedBy(ForGeolocatingIps)` now takes coordinates and country from Vaier's own database whenever it can answer, keeping the ASN from CrowdSec (DB-IP city-lite has none, and *"Bulgaria · Techoff Srv Limited"* is the identifying half); CrowdSec's own placement is the fallback. The domain owns the port call; `CrowdSecCliAdapter` composes the two ports when it builds each decision, so the sweep, the Security view and the Map all get one answer. The `0`/`0` sentinel is read as *no coordinates* where it is wire knowledge — in the CrowdSec adapter — so `locatable()` is simply "both coordinates present" and the carve-out is gone by removing its cause, not bypassed. A source CrowdSec did not enrich still gets a country and a pin when DB-IP knows it. No database was added or removed; CrowdSec keeps and uses its own. `AccessSource.locatable()` keeps its own carve-out — a different source, outside this issue.

**A fleet-level nudge ladder ✅ 2026-09-22 (closes [#336](https://github.com/getvaier/vaier/issues/336)).** A fresh install landed on *"No machines yet"* and said nothing about what Vaier is for, while a nudge engine that knew exactly what to do next sat scoped per machine. `FleetNudge` beside `MachineNudge`: one static factory per rung — let people in (pending access entries), add a machine, publish (the machine with the most unrouted services, while nothing is published yet), designate a backup server (offering a NAS when the fleet has one), write the survival kit (repositories exist, no kit ever written — `AppSettingsResult` now carries `survivalKitWritten` from the writer's fingerprint), set up mail — composed by `FleetNudges.forFleet(FleetSignals)`, which owns the order (a waiting person first, then the ladder) and the cap of three. `GET /fleet/nudges` on a new `FleetRestController` gathers the signals at the driving edge from existing use cases; the Explorer's fleet root renders them with the same `nudgeCard`, the two rungs that point at a machine carrying its id as `value` so `PUBLISH` and `DESIGNATE_BACKUP_SERVER` route into the existing flows, and the old empty-fleet note yields to the first rung. A finished fleet shows nothing: an empty state that means done.

**Pre-flight — "is it working?" ✅ 2026-09-22 (closes [#265](https://github.com/getvaier/vaier/issues/265)).** A single place that answers whether Vaier's own basics hold, mirroring the reverse proxy audit beside it on Settings: the wildcard verdict the boot probe already took; the certificate the front door actually presents, read off a live TLS handshake against Traefik from inside the stack through a new narrow driven port `ForInspectingConsoleCertificates` (`TlsHandshakeCertificateAdapter`, look-only trust, SNI = `vaier.<domain>`) behind `InspectConsoleCertificateUseCase` on `ReverseProxyService` — Traefik's self-signed placeholder reads as *not issued*, under 14 days left as *renewal is not happening*, no handshake as *could not reach its own front door*; whether `GetVpnClientsUseCase` answers at all; and whether this server's own `MachineDiskStanding` is breaching. `PreFlight.of(PreFlightFacts)` owns every verdict and the summary sentence; `GET /settings/pre-flight` composes the facts at the driving edge; the page paints nothing when the list is empty. Not checked, on purpose: the reverse proxy config (the audit already does, same page), mail sending (no record of a failed send exists; a guess is worse than silence), and the Dex crash-loop, which no panel behind the door can show.

**One backup path, not two ✅ 2026-09-22 (closes [#335](https://github.com/getvaier/vaier/issues/335)).** The Explorer had already folded the borg machinery — Provision, Authorize a host, Setup script and the coordinates sit under named folds on the backup server's entry, **New repository** is gone from every surface, retention and compression never appear on a job's face — but the docs still taught the nine-concept path first. `README.md`'s row and `web/index.html`'s card now say *Tick what matters, Vaier does the rest*, naming borg once as what is under the hood; `docs/BACKUP.md` opens with that one path and a one-paragraph *how it works*, and everything about servers, repositories, authorization and jobs is filed under **Under the hood: recovery and adoption reference**, with the two stale sentences about a Server operations section and a New repository button corrected. The machine's `backup` entry gained the one verb the acceptance named, **Back up more**, which only opens the files. Nothing was removed.

**First-run admin claim, slice 1 ✅ 2026-09-22 (slice 1 of [#264](https://github.com/getvaier/vaier/issues/264)).** Nobody could sign in to a fresh install until they had registered an OAuth app somewhere else: with no provider configured `dex-init` exited 1 rather than write a connector-less config, so the console sat dark behind a Dex that never started. That is the slowest and least rewarding first task Vaier asks of anyone, and it lands before they know the rest of the stack even came up. With **zero** providers configured `dex-init` now opens Dex's own `local` connector for exactly one account — the **first-run password** — minted once (the file is the truth across restarts), bcrypt-hashed into Dex's config by an `htpasswd` from an `apk add apache2-utils` that happens in that branch and nowhere else, named `VAIER_ADMIN_EMAIL`, else `ACME_EMAIL` (so a later provider sign-in under the operator's own address finds the admin it already is), else `admin@<domain>`, and left at `/vaier/config/first-run-password` (owner 1000, mode 0600) on the `./vaier/config` mount dex-init gained for it. Vaier reads it back through `ForReadingFirstRunPassword` → `FirstRunPasswordFileAdapter` behind `GetFirstRunPasswordUseCase` on `UserService`, and `StartupLifecycleRunner` logs `FirstRunPassword.banner(domain)` **last**, after the lifecycle, so the bordered block with the console URL, the email and the password is the bottom of `docker compose logs vaier`. The sign-in page carries one more `<!--provider:local-->` block, *Sign in with the first-run password*, deleted by the same template surgery that already removes an unconfigured provider's button — so the page still shows exactly the doors that exist. **Handover:** once a provider exists, a store whose every admin is a **first-run account** counts as locked out (`AccessRoster.needsConfiguredAdmin`), so startup restores `VAIER_ADMIN_EMAIL` to admin, or logs an error naming that fix; the `vaier` container now also receives `VAIER_OIDC_GITHUB_CLIENT_ID`, without which a GitHub-only install read as having no provider.

**The claim is a domain rule, not a seeding step.** `AccessRoster.claimsFirstAdmin(provider)` is the whole decision — the connector id is `local` and the store holds no **admin** — and `UserService.verify` upserts that identity as ADMIN ahead of the ordinary first-sighting path, so the person who reads the log becomes the admin by walking through the door rather than by being named in `.env` first. Once any admin exists the claim is spent: a local sign-in for some other email lands **pending** like any other newcomer, and a provider sign-in never claims. `AccessEntry.KNOWN_PROVIDERS` gained `local` so the entry records how it got in. The boundary the door leans on is host access — reading that log means holding a shell that can already read every secret in the stack — and it is self-closing: the moment either provider's id *and* secret are set, `dex-init` writes no password DB and `rm -f`s the file, so the door is exactly as old as the gap it fills. One drift fixed on the way: `ConfigResolver.isSocialAuthAvailable()` tested only the Google client id, so a GitHub-only install — legal since #332 — read as *no social login*; it now answers on either client id. Slice 2 — adding a provider from the Settings UI — shipped 2026-09-23 (below).
...
**Backlog — nothing tells an operator before they update.** The guard fires at `docker compose up`, on the operator's terminal, which is the last honest moment. Vaier itself never says "your `.env` is missing a secret this release added", and it is well placed to: it can read the compose file and the environment it was started with. Deferred as a separate feature, since the useful version of it is part of making **self-update** carry the whole stack (#343) rather than a standalone warning about a file the operator would then have to fix by hand anyway. **Update (§6.61):** the self-update now runs the target release's `install.sh`, which adds the missing secret itself. That leaves only the hand-fetched-compose path unwarned.
...
**Add a sign-in provider from Settings ✅ 2026-09-23 (slice 2 of [#264](https://github.com/getvaier/vaier/issues/264)).** Inviting a second person no longer means SSH and `.env`: **Settings → Sign-in** shows each provider's standing (not set up / set in .env / set here), the exact Dex redirect URI with a copy button, a link to the provider's console, and client id + secret fields. **Save** waits for the apply and reports what happened. `UserService` (new `GetSignInProvidersUseCase`, `AddSignInProviderUseCase`) validates the pair in `ProviderCredentials` (a strict `[A-Za-z0-9._-]` charset, because both values land in a root shell's heredocs and Dex's YAML), writes `SignInSettings` to `./vaier/config/sign-in-providers.env` through `SignInSettingsFileAdapter`, and hands off to the pure-domain `SignInRenderers`: re-run `dex-init`, restart Dex only if it exited 0, then the same for `oauth2-proxy-init` → oauth2-proxy. A failed renderer restarts nothing onto its output; the previous file is put back and re-rendered, and the renderer's own log tail is the error the operator reads. **Mechanism:** docker-proxy's haproxy template now denies every container start *except* `dex-init` and `oauth2-proxy-init` by exact name (create stays denied, so a start only re-runs the fixed renderer); `DockerContainerAdapter` gained `ForRerunningContainers` (start → wait → exit code → log tail). Both renderers read the file with `sed`, strip values to the same charset, and never `source` it. **.env wins** per provider when it holds both halves; the `vaier` container learns only *whether* each secret is set (`*_CLIENT_SECRET_PRESENT`, via `${…:+yes}`), never the secret. **No lockout:** the file also holds the **first-run door** open when the first provider is added this way, so the first-run password keeps working beside the new button until an admin signs in through a provider — `SignInSettings.closesFirstRunDoorOn(entry)` decides, and `UserService.verify` then closes it off-thread (`ForRunningInBackground`), once per boot, never failing the forward-auth answer. `ConfigResolver.isSocialAuthAvailable()` also counts a provider from the file, and the access store's lockout check keys on the door (`isFirstRunDoorOpen`) rather than on provider presence. An install that never opens Settings renders exactly as before. Still open on #264: the claim-token shape of the original proposal (slice 1 shipped a log-printed password instead) and removing a provider from Settings.
...
Whether anything is worth sending at all is `ErrandReport`'s decision: an answer that is blank or exactly `NOTHING_TO_REPORT` is silent — no mail, no conversation turn, no nudge — because a watch that mails every morning to say all is well gets filtered within a week, and the one morning it mattered gets skimmed along with the rest. Otherwise the operator is mailed via the new `ForSendingAdminNotification.sendTo`, the answer becomes the first turn of a fresh conversation — every run starts a new session, forgetting the operator's kept conversation before Marvin sets off (2026-09-19: a daily briefing had stacked five mornings' reports in one thread), so the next question already knows what was found and nothing older, and Vaier publishes `errand-reported` on the new `chat` SSE topic so an open pane redraws without being asked. An errand that fails outright (a dead API, an unreachable machine) still moves on to its next time, marked `failed` rather than retried — nobody is watching, so a retry loop against a paid API would be the worse bug. The Marvin menu gained "Marvin's errands (N)", listing rhythm, instruction, next run and last outcome with a cancel button on each (`GET /chat/errands`, `DELETE /chat/errands/{id}`).

**Addendum ✅ (2026-09-25): an errand may propose an action, by mail — slice 1.** Errands could only read; now `ErrandRunner` also offers the six **Chat action**s, through `ChatActions.mailedOffers(operator)` (`rest/`, the card's dispatch — `canonical` + `run` — pulled out of `ChatRestController` so the card's click, the mailed yes and the errand's proposal share one path). Calling one during an errand runs nothing: `ChatService.mail` (`MailConfirmationUseCase`) mints a **Mailed confirmation** (`domain.MailedConfirmation`: 32 SecureRandom bytes, URL-safe, only the SHA-256 digest kept; lives 24 h; taken once) and mails the errand's operator one message with the sentence and an **approval link**, `https://vaier.<domain>/chat/approvals/<token>`. The confirmation is kept in `vaier/config/mailed-confirmations.yml` (`MailedConfirmationFileAdapter`) only once the mail went, so it survives a redeploy. `domain.MailedConfirmations` owns every decision: constant-time token match, expiry, the operator check, and a roof of three waiting per operator so a watch that finds ten things mails three. `ChatApprovalRestController` serves `GET /chat/approvals/{token}` (looks, never runs — mail scanners prefetch), `POST …/{token}` (takes it and runs it through `ChatActions.run`) and `POST …/{token}/decline`, each behind the normal sign-in chain like every other `/chat` route; used, expired or another operator's links all get the same plain page. What became of it is remembered in the operator's thread, as a card's outcome is. `ChatPrompt.forErrand` now lists the actions and tells Marvin a yes is mailed for, to propose only when what he found clearly calls for it, and never to say it is done. A silent watch still sends nothing.

- **Backlog:** fold a run's mailed confirmations into its report mail instead of one mail each; say in the Chat pane which confirmations are waiting; a hard per-run cap if the per-operator roof of three turns out too loose.

**Addendum ✅ (2026-09-25): Marvin may change fleet state after asking — slice 2, OS updates.** A seventh **Chat action**, `upgrade_os(machine)`, and the Explorer button it needs so the rule "every Chat action is an Explorer verb" still holds: **Install OS updates** in a machine's pane head, offered where Vaier can open a session (SSH access, a credential, an SSH server). `UpgradeOsUseCase` on `MachineService` (no new service). The domain type `OsUpgrade` owns every decision: one short probe (`id -un`/`id -u`, `sudo -n true`, `command -v apt-get`/`dnf`) judges whether Vaier can get root — logged in as uid 0, or passwordless sudo; the borg-only sudoers grant from **Prepare client** does not count — and which package manager to use; a machine with neither root nor apt/dnf is refused with a `ConflictException` (409) naming why. apt runs `export DEBIAN_FRONTEND=noninteractive && apt-get update && apt-get -y -o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confold upgrade` (never dist-/full-upgrade, never autoremove; the two dpkg options keep a changed config file instead of hanging on its prompt); dnf runs `dnf -y upgrade`; either is wrapped in `sudo -n sh -c '…'` when not root. The outcome counts the packages changed from apt's "N upgraded, M newly installed" line or dnf's transaction summary, and reads a due reboot from `/var/run/reboot-required` or `dnf needs-restarting -r`. Vaier never reboots. **Long-running:** the probe is on the caller's thread (≤ 20 s, well inside any proxy timeout), the upgrade itself runs through `ForRunningInBackground` under a 30-minute exec deadline, so `POST /machines/{id}/os-upgrade` answers 202 and the card's click and the mailed approval both answer at once. It settles as `os-upgrade-settled` on the `vpn-peers` SSE stream (the Explorer's toast), a failure is mailed to the admins (notify only on trouble), and when the yes came through Chat the settlement sentence joins that operator's thread via the stage `UpgradeOsUseCase` returns. `ReadOnlyCommand` already allowed `apt list --upgradable` and `dnf check-update`, so Marvin's read side needed no change.

**Addendum ✅ (2026-09-26): Marvin may change fleet state after asking — slice 3, a published service's own API.** The operator decided (2026-09-25) that Marvin may use published services' own APIs: reads are free, writes wait for a yes. A new **Chat tool**, `read_service(service, path)`, in `ChatReads` and in `ChatTool.whileNobodyIsWatching()` so errands have it too — limited (operator's decision, 2026-09-26) to a **free read**: a GET on a short path allowlist in `ServiceCall` (openHAB `/rest/…`; OpenSprinkler `/jc` `/jo` `/js` `/jp` `/jn`, any query), keyed on path alone since nothing Vaier already holds names a service's kind reliably (openHAB's own sign-in reads NONE; OpenSprinkler's `ipas=` fingerprint lags boot), and any other GET refused with a pointer to `call_service`, because OpenSprinkler's `/cm` switches a station on with a GET; and an eighth **Chat action**, `call_service(service, method, path, body?)` — GET, POST, PUT, PATCH or DELETE, a GET without a body — proposed as a card live and as a **mailed confirmation** in an errand — the first action with no Explorer button, deliberately: the service's own UI is its button (`ChatAction`'s Javadoc says so). Both are a **Service call**: `ServiceCall` (domain) validates the path (relative, no scheme/host, no `.`/`..` segments even percent-encoded, no space/#/backslash), the method allow-list (a free read for `read_service`; GET/POST/PUT/PATCH/DELETE for `call_service`), the 64 000-character body cap, the content type (JSON when the body is shaped like JSON, else `text/plain`), and the card's sentence (*"POST to openhab on Colina 27 /rest/items/PoolPump with body "ON"."*, a body over 200 characters cut in the sentence only). `ServiceCallAnswer` renders what came back — text/JSON through, 16 000-character cap that says so, binary as "binary, N bytes", no headers kept — and the outcome of a yes: the status on 2xx, else a refusal with a 300-character snippet. `PublishedServiceReference` resolves a name the way `published_services` names services — address, name + machine, or a name only one service has; a shared name (two openHABs) is refused with the addresses. **Route:** the call goes straight to the route's backend (`ReverseProxyRoute.call` → `originUrl()` + path) through the new `ForCallingServices` port and `HttpServiceCallAdapter` (JDK client, redirects never followed, 256 KiB read, 20 s), carrying the `Authorization` `/authz/verify` would hand the service for the operator — personal service credential, else shared (`ServiceCredentials.credentialFor`). Going through the public URL was rejected: it needs an oauth2-proxy session Vaier does not have for itself, and would need a new unauthenticated route. `ReadServiceUseCase` / `CallServiceUseCase` on `PublishingService` (no new service). `ToolParameter` gained `optional`, so `body` may be left out; the mailed-confirmation file carries the body in the proposal's arguments. `ChatReads.offers` now takes the operator, since a read carries their credential.

- **Backlog:** HTTPS backends with a self-signed certificate (DSM on :5001, say) cannot be called — the JDK client verifies the chain; an opt-in trust-this-backend flag would fix it. A service call does not re-check the operator's **access rule** for the service; Chat is admin-only today, and an admin always passes, but a non-admin Chat would need it.
- **Backlog:** an OS-updates-pending mark on each machine from the five-minute sweep, so the button is offered only where there is something to install; a reboot verb, deliberately not in this slice; apk/pacman/zypper.
...
### 6.57 The edge answers from memory: the CrowdSec bouncer becomes Traefik's plugin in stream mode ✅ (implemented 2026-09-22)
...
### 6.58 The server absorbs the infrastructure's slowness: one read of the tunnel per tick, one read of the block list per sweep, one wave at boot ✅ (implemented 2026-09-22)

**The ask, in the operator's words:** "make the vaier server absorb slowness in the infrastructure and the
web fast by utilizing threads and sse." Layer two of "Explorer is slow to load" (§6.57 was the edge).

**What was slow, measured.** With the edge fixed, the boot's own reads still cost the JVM ~1.9 s in the
burst. `MachineService.getAllMachines()` reads the tunnel — two Docker execs into the wireguard container
(`wg show interfaces`, `wg show wg0 dump`) — on every call, and it has 22 callers; `BackupRestController`
called it once *per row* (`machineNameOf`, `storeLabel`), so `/backup-jobs` and `/backup-repositories` cost
0.6 s each alone and 1.7 s under the burst. `/security/decisions` ran `cscli decisions list` for every
page load: 0.3–1.3 s. And the shell fetched its thirteen boot reads as two serial chains — four round
trips deep for the fleet, three for the backups — on a phone that pays ~100 ms per link.

**The pattern, and where it lives.** The path already existed: `PeerStatsScheduler` reads the tunnel every
ten seconds for the peers stream, and `BreachAttemptWatcher` reads the block list every five minutes for
the security stream. So each **driven adapter memoises its last successful read for exactly that
interval** — `WireGuardVpnAdapter.TUNNEL_READ_MEMO` (10 s), `CrowdSecCliAdapter.BLOCK_LIST_MEMO` (5 min) —
and forgets it the moment *it* writes (a peer removed or re-addressed, a block lifted). A request inside
the window gets the tick's answer without waiting on an exec; the scheduler's own read refreshes it. A
failed read is never memoised. This is infrastructure behaviour in an adapter (a clock, a record, a
`volatile`), not a domain decision: nothing about *what* the tunnel or the list means moved. A peer
added by another path shows on the next tick — at worst ten seconds in which `PeerRoster` lists it as
absent, which a peer with no handshake yet is.

**The controller** reads the fleet and the jobs once per list and hands them to the per-row mappers.
**The shell** fires each of its three chains as one `Promise.all` wave.

**What was deliberately not built.** A single "boot snapshot" endpoint. On HTTP/2 thirteen parallel
...
### 6.61 Vaier's own update carries the whole stack ✅ (implemented 2026-09-23, closes [#343](https://github.com/getvaier/vaier/issues/343))

**Settings → Update Vaier** used to replace the image and nothing else. A fix that lived outside the image (a compose change, an init container, a sign-in template) never arrived through it. On 2026-09-23 that turned into a live defect: docker-proxy's rules now allow the two init containers to start, and an install that took only the new image got a 403 from **Settings → Sign-in**.

**What it does now.** The release image carries the commit it was built from as `org.opencontainers.image.revision`, stamped by CI and empty on a local build. After the pull, the update script reads that commit and runs *that commit's* `install.sh` in the install directory. The target release decides which files it needs, fetches them from its own tarball, and adds any secret it introduced: the same code as a hand re-run, not a second copy. Then the whole project runs `docker compose up -d`, and `wireguard-masquerade` is recreated because a recreated `wireguard` takes it down. The Vaier health check still decides whether the update succeeded. `install.sh` now fetches `tar.gz/<ref>` rather than `refs/heads/<ref>`, so a commit or a tag works as well as a branch.

**The rollback target grew.** Before anything moves, the script records the image digest and copies `SelfUpdateScript.RUNTIME_PATHS`. `InstallScriptCoverageTest` keeps that list equal to `install.sh`'s. If the stack doesn't come up or Vaier doesn't answer, the script restores both (whole trees, so a file the release added doesn't survive) and brings the stack up again: `ROLLED_BACK`, with `up-failed` or `no-answer` in the detail. A sync that fails puts the files back before anything is recreated: `FAILED stack-sync-failed`. `UPGRADED` names `stack@<commit>`, or `stack-not-synced` for a local build. The outcome words on disk are unchanged.

**Decided against.** `--remove-orphans`: a service that compose no longer lists could be one Vaier retired or one of the operator's own in the same project, and Vaier can't tell which. A retired sidecar left running is harmless. A list of retired services to delete would be machinery that has to be kept up to date by hand. Syncing from `main` when the image names no commit: the files could disagree with the image, which is exactly what this fixes.

**Known limit.** The script is written by the Vaier that is *already running*, so the first update onto this release is still image-only. Every update after it carries the stack.

### 6.62 Blocking an address by hand ✅ (implemented 2026-09-23, closes [#349](https://github.com/getvaier/vaier/issues/349))

**What.** #349 named the asymmetry plainly: the Security view could unban and permanently trust, but never *ban*. Filed as the weaker half of a pair with #348 (already shipped, the reversible-trust side), with a real case against building it at all — CrowdSec is the authority on who gets blocked, and a hand-maintained blocklist accumulates exactly like #348's unreviewed trust list did. The operator decided: build it, with expiry, so it self-heals the way every other decision on this view already does.

**Mechanism, mirroring the unban exactly.** `ForAddingBlocks` (`domain/port/`) joins `ForLiftingBlocks` on `CrowdSecCliAdapter` — the same container-exec transport, `cscli decisions add --ip <ip> --duration <d> --reason <marker>` beside `cscli decisions delete -i <ip>`. `BlockDuration` (domain enum: `ONE_HOUR`, `FOUR_HOURS`, `TWENTY_FOUR_HOURS`, `SEVEN_DAYS`) is the operator's four choices and nothing wider — **no permanent option**, so even the longest hand block expires. `cscliDuration()` expresses seven days as `168h`: Go's `time.ParseDuration`, which `cscli`'s flag rides on, has no unit for days at all.

**Distinguishability.** `BlockDecision.handBlockReason(adminEmail)` writes `"vaier: blocked by <email>"` into the one field `cscli --reason` actually sets on the wire shape Vaier already reads — `scenario`. `BlockDecision.handBlocked()`/`blockedByAdmin()` read it back on the same list CrowdSec's own decisions ride in, so `BlockDecisionResponse` carries `handBlocked`/`blockedByAdmin` and the Security view marks a row "blocked by you/&lt;admin&gt;" without a second field CrowdSec's wire format would have needed to grow. The same marker keeps a hand block out of `BreachAttemptRollup` — mailing "somebody is trying to break in" about a block the operator placed on purpose would be exactly the false alarm #348's own lockout-warning reasoning already refuses to send.

**Two domain refusals, both in `SourceAddress.block`, both checked before `ForAddingBlocks` is ever called.** An address inside the fleet's own `TrustedNetworks` (the VPN subnet, the Docker bridge, every relay LAN, every hand-trusted address) is refused for the same reason #329 already guards the automatic side — blocking the operator's own traffic is the console they'd fix it from. The requesting admin's own current address — resolved by `domain.CallerIp`, the same way the launchpad and `/vpn/peers/my-position` already do — is refused separately: nothing about the trusted networks catches an admin working from an ordinary home connection. Both surface as a plain `IllegalArgumentException` → `400`, exactly like every other domain refusal in this project; no new exception type was added for them.

**Idempotency, decided explicitly.** `cscli decisions add` does not reject a duplicate — it adds a second active decision, and the source stays blocked until the later of the two expires. That is "extend the block" in effect, with neither the adapter nor the domain special-casing an address that is already blocked.

**Wiring.** `BlockAddressUseCase` on `SecurityService` (no new `*Service` — same domain, same one-service rule every other Security use case already follows). `POST /security/decisions` on `SecurityRestController`, admin-only through the ordinary auth chain like every other path on this controller, reading `X-Auth-Request-Email` for the reason marker and the audit log line exactly the way `ChatRestController` already does for "who is asking". Publishes the refreshed decision list on success, the same act-then-publish pattern `liftBlock`/`trustAddress` use.
