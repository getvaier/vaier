...
- **Admin UI ✅** — the **Backups** tab (`backups.html`), reached at `#backups`, is where backup servers are registered/provisioned (setup.sh download, provision-with-polling, authorize-host), repositories are added by name (server picker + auto-generated passphrase, effective-path preview), jobs are configured, runs are triggered and read, readiness is checked (with inline authorize when the key isn't trusted), and archives are browsed. Machine pickers read `GET /machines` (peers **and** LAN servers such as NAS), not `/vpn/peers`. Locked gate state on Community.
