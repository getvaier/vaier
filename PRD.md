...
- **Connected-peer world map ✅** — Machines page exposes a **List / Map / Topology** tab switcher. The Map tab renders a Leaflet/OpenStreetMap world map (Leaflet and the markercluster plugin are self-hosted under `static/vendor/` so the page works without unpkg/CDN access). Coordinates come from a local DB-IP City Lite MMDB lookup; the database is downloaded by a `geoip-init` container into a shared `./geoip/` volume on first boot and refreshed monthly. Lookups happen via a `ForGeolocatingIps` port (`DbIpGeolocationAdapter`) and are folded into the existing `GET /vpn/peers` payload so the page makes a single `wg show dump` call per refresh. Private/RFC1918/CGNAT/IPv6-ULA endpoints are filtered out, and the map gracefully shows no markers if the DB is missing.
  ...
  - **Topology diagram ✅** — A third **Topology** tab (renamed from "Network" as the first slice of the Infrastructure merge, see backlog) renders an interactive **force-directed graph** of the whole Vaier network: the Vaier server, every VPN peer, each LAN server, and every published service are nodes in a live physics simulation (Cytoscape.js with the cola layout, both self-hosted under `static/vendor/cytoscape/` so the page works without CDN access). Nodes repel and edges pull, so the graph self-spaces; the operator can drag a node (its neighbours reflow in real time), zoom, and pan. **Published services attach to the machine that hosts them** — the host VPN peer for a peer-hosted service, the named LAN server for a LAN service, or the central Vaier hub when `hostName` is empty/`"Vaier server"` — coloured by its own health (`state` OK → green, UNKNOWN → grey, else red) with its authentication state in a hover tooltip. A service whose host machine isn't in the graph is silently dropped. Edges are coloured/dashed by connectivity status (green = up, red dashed = down, yellow = degraded, grey dashed = unknown), reusing the same machine icons and status colours as the List and Map tabs (each node icon is the machine-icon SVG baked into a per-node data URI). A status-only refresh recolours nodes in place without re-running the layout, so a 30s poll or SSE update never teleports the graph. Data comes entirely from endpoints the page already (now) calls (`GET /vpn/peers`, `GET /lan-servers`, `GET /vpn/peers/server-location`, `GET /published-services/discover`); the service↔machine join is client-side, and the graph refreshes live over both the peers and `/published-services/events` SSE streams — no new REST endpoint. Frontend only (`static/vpn-peers.html` + `vpn-peers.js` + `vpn-peers.css` + vendored Cytoscape/cola libs).
...
**Infrastructure page merge (in progress).** The Machines (`vpn-peers.html`) and Services (`published-services.html`) pages are being merged into one machine-centric **Infrastructure** page in shippable slices. **Slice 1 ✅ (this change):** the Machines page's diagram tab gained the published-service layer and was renamed Network → **Topology** (the join is a client-side helper, mirroring the existing peers+LAN-server join); the diagram itself was reworked from the hand-positioned SVG fan into an interactive Cytoscape + cola force-directed graph (drag/zoom/pan, self-spacing physics) after the fixed-radius layout crowded badly. **Slice 2 (planned):** machine cards gain an expandable services section (published + per-machine "+ publish" for discoverables), folding in the services-card body, auth toggles, version rows, and publish/delete modals. **Slice 3 (planned):** collapse the two nav entries into a single **Infrastructure** entry, delete `published-services.html`, fold its JS/CSS in, redirect old `#services`/`#vpn` hashes, and add the `Infrastructure` term to `UBIQUITOUS_LANGUAGE.md`. The Infrastructure-section rename is deliberately deferred to slice 3 — only the inner tab is "Topology" today.
