...
- **LAN-native service version** 🔲 (backlog) — services published as a bare LAN host:port (not a container) currently show no version. Probing a version from the service's own URL was considered but deferred: there is no standard version endpoint, so any heuristic would be unreliable.
