...
- **Send stored password to a live prompt ✅.** A **Send password** key on each shell's tab asks Vaier to
  write that machine's stored password straight into the PTY, so it travels vault → SSH server without the
  browser ever holding it. The safety gate is a domain rule: `PasswordPrompt.isAwaitingPassword` matches a
  password prompt **only at the tail** of the recent output (`(?is).*password(?: for \S+)?:\s*\z`), so a
  `password:` already answered upthread can't trigger a send that would echo into the screen or shell
  history. The narrow `SendHostPasswordUseCase` (on `TerminalService`) returns `SENT` / `NOT_AT_PROMPT` /
...
- **Admin UI ✅** — the **Backups** tab (`backups.html`), reached at `#backups`, is where backup servers are registered/provisioned (setup.sh download, provision, authorize-host), repositories are added by name (server picker + auto-generated passphrase, effective-path preview), jobs are configured, runs are triggered and read, readiness is checked (with inline **Authorize host** when the key isn't trusted and inline **Prepare client** when borg isn't installed), and archives are browsed. The prepare-client install, on-demand runs, and server provisioning all run detached on the host and the browser learns each one finished over a **single `backups` SSE stream** (one `EventSource('/backup-jobs/events')` handling `prepare-client-settled`, `run-settled` and `provision-settled`) — it never polls; the backend sweeps do all the host-side polling. There is no `setInterval` in `backups.js`. Machine pickers read `GET /machines` (peers **and** LAN servers such as NAS), not `/vpn/peers`. Locked gate state on Community.
