...
- [x] **The backup server, designated on a machine ✅.** The fleet's one **backup server** stops being a
  ...
  repositories on it **read-only**, with **Edit coordinates** and **Remove designation** actions and a link to
  the Backups page for the operations that poll for an outcome (provision, authorize, repositories, jobs) — those
