...
- **Connected-peer world map ✅** — Machines page exposes a **List / Map / Network** tab switcher. The Map tab renders a Leaflet/OpenStreetMap world map (Leaflet and the markercluster plugin are self-hosted under `static/vendor/` so the page works without unpkg/CDN access). Coordinates come from a local DB-IP City Lite MMDB lookup; the database is downloaded by a `geoip-init` container into a shared `./geoip/` volume on first boot and refreshed monthly. Lookups happen via a `ForGeolocatingIps` port (`DbIpGeolocationAdapter`) and are folded into the existing `GET /vpn/peers` payload so the page makes a single `wg show dump` call per refresh. Private/RFC1918/CGNAT/IPv6-ULA endpoints are filtered out, and the map gracefully shows no markers if the DB is missing.
  ...
  - **Network diagram ✅** — A third **Network** tab renders a hub-and-spoke bird's-eye view of the whole Vaier network: the Vaier server at the centre, every VPN peer radiating from it on an inner ring (with an edge back to the hub), and each LAN server branching outward from the relay peer (or the Vaier server) it sits behind. A hand-rolled SVG/HTML diagram with no new dependency; edges are coloured/dashed by connectivity status (green = connected/up, red dashed = down, yellow = degraded, grey dashed = unknown), reusing the same machine icons, status colours, and relay badge as the List and Map tabs. Data comes entirely from the endpoints the page already calls (`GET /vpn/peers`, `GET /lan-servers`, `GET /vpn/peers/server-location`) and refreshes live over the existing SSE stream — no new REST endpoint. Frontend only (`static/vpn-peers.html`).
...
- **UI** — the per-category icon (`deviceCategoryIconKind` in `vpn-peers.html`) drives the glyph for every machine on all three Machines tabs (List, Map, Network), replacing the old machine-type icon. The expanded machine card carries a **Device type** selector (`deviceCategorySelectHtml`): an "Auto-detect" option plus the twelve categories, with an "Auto-detected: …" caption when no override is set; choosing a category `PATCH`es the override, choosing "Auto-detect" clears it. The LAN-scan results picker shows each discovered host's device icon, and picking a host carries its derived category into the new LAN server via the `POST /lan-servers` `deviceCategory` field.
...
- **Internet gateway — designate a peer as the central internet egress** ✅ (closes [#174](https://github.com/getvaier/vaier/issues/174)) — by default every full-tunnel client peer (`0.0.0.0/0`) NATs out the Vaier server's own connection via the `wireguard-masquerade` sidecar. This feature lets the operator designate **one** Linux server peer (`UBUNTU_SERVER`) as the internet gateway instead, so client internet traffic is forwarded to that peer and leaves from there (a phone on the VPN in Spain can appear to be in the USA). **Pure server-side change on `wg0`:** the designated peer's `[Peer] AllowedIPs` becomes `<vpn-ip>/32,…lanCidr…,0.0.0.0/0` (`WireGuardPeerConfig.serverAllowedIps(ip, lanCidr, true)`); the previously-designated peer reverts to its non-default-route value; all other peers stay `/32`, so longest-prefix match keeps peer-to-peer and relay-LAN routing intact. Invariant: **at most one** internet gateway at a time — selecting a new one clears the old. The designation is stored as `"gateway":true` in the peer's `# VAIER:` metadata (`WireGuardPeerConfig.vaierJson(…, isGateway)`), and **only ever affects the server-side `AllowedIPs`** — it never enters the client tunnel directives, so `isOutOfDate` (which strips the `# VAIER:` comment) is unaffected (regression-tested). **Egress NAT is unconditional on every Linux server peer** (`PeerSetupScript`): the bash setup script always installs `ip_forward` + an interface-agnostic `iptables -t nat -A POSTROUTING -s <vpnSubnet> -o "$EGRESS_IF" -j MASQUERADE` (egress iface detected at runtime via `ip route show default`; no `-d` filter) and FORWARD accepts, persisted across reboot by a sibling `vaier-wg-egress-iptables.service` systemd oneshot that **re-detects** the iface at boot. The rule is dormant unless the server forwards `0.0.0.0/0` to the peer (only the one designated gateway), so it is safe on every server peer. Strict hex: eligibility is the domain predicate `MachineType.canBeInternetGateway()`; `VpnService` (new narrow `DesignateInternetGatewayUseCase`, **not** a new service) validates → reverts any current gateway → sets the new one's server `AllowedIPs` and metadata via the `ForUpdatingServerAllowedIps` / `ForUpdatingPeerConfigurations` ports. REST: `PUT /vpn/peers/{peerName}/gateway` (designate, 400 on ineligible/missing), `DELETE …/gateway` (clear), and `isInternetGateway` on the peer GET response. UI: a single-select toggle on each eligible (`UBUNTU_SERVER`) peer's card, with a tooltip noting the peer must have run its setup script for the egress rules to carry traffic. **V1 limitation:** a single, all-clients gateway. See backlog below for per-client selection and multiple simultaneous gateways.
  - **Backlog (#174 follow-ups):** per-client gateway selection / multiple simultaneous internet gateways via policy routing; IPv6 default route (`::/0`); a client-side kill-switch; and gateway health monitoring (auto-failover when the designated peer is down).
