...
- **Caveat:** `ApiError` is the default error shape, not yet universal — some endpoints still answer validation/not-found/conflict with a bare status code and no body (e.g. LAN-server and peer rename/delete), and the enterprise-gate `402` (`EnterpriseLicenseInterceptor`) is an intentional non-`ApiError` shape.
