...
Marvin can also ask a published service itself: *"is the pool pump on?"* reads openHAB's `/rest/items/PoolPump/state`. A **free read** (`read_service`) is one GET of a path Vaier knows changes nothing, and it needs no click — it works in errands too. The list is short and kept by path: openHAB's `/rest/…`, and OpenSprinkler's status pages `/jc`, `/jo`, `/js`, `/jp` and `/jn`, with any query. Any other GET is refused, because a GET is not always harmless — OpenSprinkler switches a station on with one — and Marvin is told to propose it instead. Everything else through a service's API (`call_service`: GET, POST, PUT, PATCH or DELETE) is a **Chat action** like any other, proposed as a card or, during an errand, as a mailed confirmation; nothing is written until you say yes, and the card says exactly what will be sent — *"POST to openhab on Colina 27 /rest/items/PoolPump with body "ON"."* A long body is cut in the sentence only; the yes sends all of it. The outcome is the service's status, plus the start of what it said when it was not a success.

**How it reaches the service.** Vaier calls the service's **backend** — the address and port the route points at, over the tunnel, exactly where the sign-in look and the version probe go — never its public name, which would put Vaier's own sign-in in the way. It carries the same `Authorization: Basic …` that `/authz/verify` would hand the service for you: your **personal service credential**, else the **shared** one, else nothing (see [Service credentials](AUTH.md#service-credentials)). A service with no credential answers with its own refusal, usually 401, and Marvin says so.
