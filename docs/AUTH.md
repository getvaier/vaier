...
**Marvin uses it too.** When Chat reads or writes a service's own API, Vaier calls the backend with the same credential it would hand the service for the operator asking — see [A published service's own API](CHAT.md#a-published-services-own-api).
